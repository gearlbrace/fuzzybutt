...
/afkdel [ENABLE|DISABLE] - Auto delete AFK status messages after 1 minute (Only for group admin). Default is **Enable**.
Just type something in group to remove AFK Status."""

# chat_id -> [{"msg_id": int, "delete_at": datetime}]
pending_cleanups = {}


async def track_for_cleanup(chat_id, message_id):
    if not await is_afk_cleanmode_on(chat_id):
        return
    pending_cleanups.setdefault(chat_id, []).append(
        {"msg_id": message_id, "delete_at": datetime.now() + timedelta(minutes=1)}
    )
...
async def _cleanup_worker():
    while True:
        ...
        for chat_id in list(pending_cleanups):
            now = datetime.now()
            entries = pending_cleanups[chat_id]
            due = [e["msg_id"] for e in entries if e["delete_at"] <= now]
            if due:
                try:
                    await app.delete_messages(chat_id, due)
                except Exception:
                    pass
                pending_cleanups[chat_id] = [
                    e for e in entries if e["delete_at"] > now
                ]
            if not pending_cleanups[chat_id]:
                del pending_cleanups[chat_id]


asyncio.get_running_loop().create_task(_cleanup_worker())
